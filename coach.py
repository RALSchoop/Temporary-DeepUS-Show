"""Module which defines helpful functionality for training.
...
def test_single_epoch(epoch_idx: int, avg_batch_amount: int, test_loader: torch.utils.data.DataLoader,
                      model: torch.nn.Module, loss_fn: torch.nn.modules.loss._Loss, writer: SummaryWriter,
                      device=None):
    ...
    with torch.no_grad():
        for batch_idx, samples in enumerate(test_loader):
            ...
            if batch_idx % avg_batch_amount == avg_batch_amount - 1:
                ...
                print(f'Test Batch {batch_idx + 1}, Latest Average Test Loss: {last_avg_loss}')
...
def train_single_epoch(epoch_idx: int, avg_batch_amount: int, train_loader: torch.utils.data.DataLoader,
                       optimizer: torch.optim.Optimizer, model: torch.nn.Module,
                       loss_fn: torch.nn.modules.loss._Loss, writer: SummaryWriter,
                       device = None):
    """Trains the model for a single pass over the whole train loader.

    Arguments:
     - epoch_idx: Index of the current epoch (0-based).
     - avg_batch_amount: The amount of batches over which to calculate the average loss.
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        if batch_idx % avg_batch_amount == avg_batch_amount - 1:
            ...
            print(f'Training Batch {batch_idx + 1}, Latest Average Train Loss: {last_avg_loss}')
...
def generate_batch_display(data, targets, h_data):
    """Creates lists of matplotlib.pyplot.figure instances for displaying the batch.

    Three image sets are generated:
     - Data is plotted as a graph.
     - Data is migrated and post-processed to create a manual reconstructed image set.
     - Target images.
    """
    input_imgs = deepus.torch_image_formation_batch(torch.real(deepus.torch_fkmig_batch(data, h_data)))
    ...
    for batch in range(data.size(0)):
        ...
        plt.imshow(torch.squeeze(input_imgs, 1)[batch, :h_data['nz_cutoff'], :].numpy(), aspect='auto', cmap='gray')
        input_img_figs.append(input_img_fig)
        plt.close(input_img_fig)

        targetfig = plt.figure(batch + 2 * data.size(0))
        plt.imshow(torch.squeeze(targets, 1)[batch, :h_data['nz_cutoff'], :].numpy(), aspect='auto', cmap='gray')
