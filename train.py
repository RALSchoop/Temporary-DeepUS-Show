...
@TODO: Consider all parameters worth varying.
"""
...
# More important it seems that cuFFT doesn't work with dtype float 64
torch.set_default_dtype(torch.float32)

# Better to use still: torchvision.transforms.Compose([torchvision.transforms.ToTensor()]).
deepus_dataset = deepus.UltrasoundDataset(
        '/export/scratch2/home/rals/Data/DeepUS/TrainingData/CIRS073_RUMC',
...
# Set the batch size.
train_loader = torch.utils.data.DataLoader(deepus_dataset, batch_size=(batch_size := 1),
                                           sampler=train_sampler)
test_loader = torch.utils.data.DataLoader(deepus_dataset, batch_size=batch_size,
...
optimizer = torch.optim.Adam(model.parameters(), learning_rate := 1e-2, amsgrad=True)

# Log settings about run.
training_settings_path = os.path.join(writer.get_logdir(), f'training_settings.txt')
...
for epoch in range(epochs := 70):
    ...
    print() # Just for \n.
    train_loss = coach.train_single_epoch(epoch, math.floor(len(train_loader) / 6),
                             train_loader, optimizer, model, loss_fn, writer,
                             device=c_device)
    print()
    test_loss = coach.test_single_epoch(epoch, math.floor(len(test_loader) / 6),
                            test_loader, model, loss_fn, writer, device=c_device)
